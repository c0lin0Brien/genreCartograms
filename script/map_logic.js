...
export function do_update(i=11, style) {
    setTimeout(function () {
        // this sets the value to use for scaling, per country. Uses square root scaling
        carto.value(function (d) {
            var countryName = d.properties.name;
            if (style) {
                var val = style_data.get(countryName)[i]
            } else {
                var val = genre_data.get(countryName)[i];
            }
